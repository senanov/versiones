...
class NovedadModel
{
    ...
    public function registrarNovedadModel($datos)
    {
        ...
        if ($datos["id"] == 2 || $datos["id"] == 4) {
            ...
            $this->db1->query("INSERT INTO novedad (documento, id_tipo_novedad, grupo, numero_ficha, fecha, centro_actual, centro_traslado, ciudad_actual, ciudad_traslado, acta, motivo, estado) VALUES (:documento, :tipo_novedad, :grupo, :ficha, :fecha, 'N/A', 'N/A', 'N/A', 'N/A', :acta, 'N/A', :estadoNov)");
            ...
            $this->db1->bind(":acta", $datos["acta"]);
            $this->db1->bind(":estadoNov", $datos["estadoNov"]);
        ...
        if ($datos["id"] == 1 || $datos["id"] == 3 || $datos["id"] == 5) {
            ...
            $this->db1->query("INSERT INTO novedad (documento, id_tipo_novedad, grupo, numero_ficha, fecha, centro_actual, centro_traslado, ciudad_actual, ciudad_traslado, acta, motivo, estado) VALUES (:documento, :tipo_novedad, :grupo, :ficha, :fecha, 'N/A', 'N/A', 'N/A', 'N/A', :acta, :motivo, :estadoNov)");
        ...
        if ($datos["id"] == 6) {
            ...
            $this->db1->query("INSERT INTO novedad (documento, id_tipo_novedad, grupo, numero_ficha, fecha, centro_actual, centro_traslado, ciudad_actual, ciudad_traslado,acta, motivo, estado) VALUES (:documento, :tipo_novedad, :grupo, :ficha, :fecha, :centroa, :centrot, :ciudada, :ciudadt, :acta, :motivo, :estadoNov)");
    ...
    //se obtiene todas las novedades
    public function consultarNovedadesModel()
    {

        $this->db->query("SELECT * FROM usuario INNER JOIN novedad ON usuario.documento_usuario=novedad.documento INNER JOIN tipo_documento ON usuario.tipo_documento=tipo_documento.id_tipo INNER JOIN ficha ON novedad.numero_ficha=ficha.codigo_ficha INNER JOIN jornada ON ficha.id_jornada=jornada.id_jornada INNER JOIN trimestre ON ficha.id_trimestre=trimestre.id_trimestre INNER JOIN sede ON ficha.id_sede=sede.id_sede INNER JOIN programa ON ficha.id_programa=programa.id INNER JOIN tipo_novedad ON novedad.id_tipo_novedad=tipo_novedad.id_novedad WHERE novedad.estado = 1");

        $this->db->execute();
        return $this->db->registros();
    } #fin del metodo

    //se obtiene una novedad recibiendo como parametro el numero de documento
    ...
    public function eliminarNovedadModel($id)
    {
        $this->db->query("UPDATE novedad SET estado = '2' WHERE novedad.documento = :id;");
        $this->db->bind(":id", $id);
